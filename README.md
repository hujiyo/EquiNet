...
- 2026.2.13:**标签机制重构** - 将二分类标签从简单涨幅阈值预测改为强势信号检测机制判断，设计了多种标签标记条件情况，替代原有的单一涨幅阈值机制。原来的二分类强迫模型像人类交易员一样做决策，新的标签机制则减少了"有趋势但被标0"的矛盾样本，明确了模型的预测目标为短期强势信号。扩展了模型规模。新增实战收益率评估指标，修正收益率计算逻辑。收益率均值最高达到3.4%,均值为2%-2.5%（TopK%收益率不是实战收益率，需要谨慎参考）。
