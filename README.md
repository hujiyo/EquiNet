...
## 项目简介
 ...
 - EquiNet 基于 PyTorch 的深度学习项目，支持灵活参数配置和高效GPU训练。通过历史数据建模为未来3天内股票是否上涨进行打分（0-1之间的连续值）。
 - **数据泄露的解决方案**：吸取了之前数据泄露的教训，最新的训练方法会将数据集中最近的80天数据冻结作为评估集，并通过去交叠的机制避免了数据泄露，同时确保了评估的一致性和可重复性。
 - **预测性能良好**：在大量测试训练过程中我发现AUC的高低对于阈值的选取过于敏感，大部分时刻并不能很稳定的反映模型的实际排序打分能力，因此最新的评估机制我最终还是返璞归真，决定完全由Top-k%的收益率来进行测试（实验默认取1%），在实验条件参数以及实验数据集下，最新的模型总体收益率在50%的情况下能超过1%，40%能超过0%，5%能超过1.5%，5%以上低于0%，在大部分情况下，模型的收益率高于实际随机基准（约-0.55%），体现了一定的实战预测能力。
 - **新增教师模型标签纠偏机制**：能在收益率表现较优的模型的基础之上对现有只经过真实标签训练的模型进行上限的拔高，通常能将模型的收益率进一步提升0.2%-0.6%
...
EquiNet/
├── src/
│   ├── predict/		#模型训练好后，将你要预测的股票按照格式做成csv放在本文件夹下
│   ├── data/                	# 存放319个.csv股票数据文件
│   ├── out/                 	# 训练输出的模型权重
│   ├── train.py         	    # 主训练脚本
│   ├── run.py    	          # 使用模型脚本
│   ├── config.py        	    # 统一配置文件
...
- 数据目录：`./data`
- 价格区间：1 ~ 6 元
- 每个`.csv`文件对应一只股票，包含420天数据，共319只股票。
- 字段说明（共8列）：
  - `time`：日期（格式：`20231211`，即YYYYMMDD）
  - `start`：开盘价
  - `max`：最高价
  - `min`：最低价
  - `end`：收盘价
  - `volume`：股票成交量
  - `exchange`：交易所代码（新增字段，暂未使用）
  - `rate`：汇率/比率（新增字段，暂未使用）
- 数据来源：通达信
...
2. **下载数据集**
   ...
   下载后将数据集放在`./data`目录下。每个`.csv`文件对应一只股票，包含420天数据，共319只股票。
...
- 2026.1.8:特征提取层改为FFN结构，修正收益率计算规则，优化采样机制：动态索引生成与循环采样支持。修正前收益率最高达到3%，修正后最高达到2%。
